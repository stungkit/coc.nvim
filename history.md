...
- Extension loader and `loadExtension` accept `sourceCode` with an
  `extensionRoot` to load a plugin from its complete CommonJS bundle
  (generated by coc-test) instead of resolving the entry through the
