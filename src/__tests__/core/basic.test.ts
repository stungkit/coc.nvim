...
describe('createFloatFactory()', () => {
  ...
  it('should create FloatFactory', async t => {
    ...
    let curr = await shared.getFloat()
...
describe('showMessage()', () => {
  ...
  it('should showMessage on vim', async t => {
    ...
    await shared.waitValue(async () => shared.getCmdline().then(s => s.includes('my message')), true)
...
describe('contentProvider', () => {
  ...
  it('should react on change event of document content provider', async t => {
    ...
    await shared.waitValue(() => doc.attached, false)
    emitter.fire(URI.parse('jdk://1'))
  })
  ...
  it('keeps the latest refresh content when an older refresh resolves late', async t => {
    ...
    try {
      ...
      second('new content')
      first('stale content')
      await shared.waitFor('getline', ['.'], 'new content')
...
async function triggerAutocmd(id: number): Promise<void> {
  await getCurrentPlugin().cocAction('doAutocmd', id)
}
...
describe('create terminal', () => {
  ...
  it('should use cleaned env', async t => {
    ...
    await shared.waitFor('eval', [`join(getbufline(${terminal.bufnr},1,'$'),'\n')`], /\S/)
    let lines = await buf.lines
  ...
  it('should have exit code', async t => {
    ...
    await shared.waitFor('bufloaded', [terminal.bufnr], 0)
  ...
  it('should create terminal', async t => {
    ...
    terminal.dispose()
    await shared.waitValue(() => terminal.bufnr, undefined)
