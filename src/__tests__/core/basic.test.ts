...
editorSuite('moveTo()', () => {
  it('should moveTo position', async t => {
...
editorSuite('getCursorScreenPosition()', () => {
...
editorSuite('createFloatFactory()', () => {
  it('should create FloatFactory', async t => {
    ...
    let win = await session.getFloat()
    ...
    let curr = await session.getFloat()
    assert.strictEqual(curr.id, win.id)
  })
})

editorSuite('showMessage()', () => {
  it('should showMessage on vim', async t => {
    ui.echoMessages(nvim, 'my message', 'more', 'more')
    await session.waitValue(async () => session.getCmdline().then(s => s.includes('my message')), true)
    let cmdline = await session.getCmdline()
    assert.match(cmdline, /my message/)
  })

  it('should get messageLevel', t => {
...
editorSuite('getSelection()', () => {
  it('should return null when no selection exists', async t => {
    let res = await ui.getSelection(nvim, 'v')
    assert.strictEqual(res, null)
  })

  it('should return range for line selection', async t => {
    await nvim.setLine('foo')
    await nvim.input('V')
  ...
  it('should return range of current line', async t => {
...
editorSuite('selectRange()', () => {
  it('should select range #1', async t => {
  ...
  it('should select range #2', async t => {
    await nvim.call('setline', [1, ['foo', 'b']])
  ...
  it('should select range #3', async t => {
...
editorSuite('doKeymap()', () => {
  it('should not throw when key not mapped', async t => {
    await keymaps.doKeymap('<C-a>', '')
  })

  it('should invoke exists keymap', async t => {
    let called = false
...
editorSuite('registerKeymap()', () => {
  it('should getBufnr', t => {
    assert.strictEqual(getBufnr(3), 3)
    assert.strictEqual(getBufnr(true), 0)
  })

  it('should getKeymapModifier', t => {
  ...
  it('should register insert key mapping', async t => {
    ...
    let res = await nvim.call('execute', ['verbose imap <Plug>(coc-test)']) as string
    assert.match(res, new RegExp('coc#_insert_key'))
  })

  it('should register with different options', async t => {
    let called = false
    let fn = () => {
      called = true
      return ''
    ...
    assert.match(res, new RegExp('coc#rpc#notify'))
    await nvim.eval(`feedkeys("\\<Plug>(coc-test)")`)
    await session.waitValue(() => called, true)
  })
})

editorSuite('registerExprKeymap()', () => {
  it('should visual key mapping', async t => {
    await nvim.setLine('foo')
    let called = false
    let fn = () => {
      called = true
  ...
  it('should register expr insert key mapping', async t => {
...
editorSuite('registerLocalKeymap', () => {
  ...
  it('should regist insert mode keymap', async t => {
    let bufnr = await nvim.call('bufnr', ['%']) as number
  ...
  it('should regist cmd keymap', async t => {
    let bufnr = await nvim.call('bufnr', ['%']) as number
    let called = false
...
editorSuite('watchers', () => {
  it('should watch options', async t => {
    ...
    await session.waitValue(() => times, 1)
    disposable.dispose()
  ...
  it('should watch global', async t => {
  ...
  it('should show error on watch callback error', async t => {
    let called = false
    let fn = () => {
      called = true
    ...
    nvim.command('set showmode', true)
    await session.waitValue(() => called, true)
    let line = await session.getCmdline()
    assert.match(line, new RegExp('Error on OptionSet'))
    called = false
    workspace.watchGlobal('y', fn, disposables)
    await nvim.command('let g:y = 2')
    await session.waitValue(() => called, true)
    line = await session.getCmdline()
    assert.match(line, new RegExp('Error on GlobalChange'))
  })
})

editorSuite('contentProvider', () => {
  it('should not throw for scheme not registered', async t => {
    await workspace.contentProvider.onBufReadCmd('not_exists', '')
  })

  it('should register document content provider', async t => {
  ...
  it('should react on change event of document content provider', async t => {
    let text = 'foo'
    ...
    await session.waitFor('getline', ['.'], 'bar')
    await nvim.command('bwipeout!')
    await session.waitValue(() => doc.attached, false)
    emitter.fire(URI.parse('jdk://1'))
  })

  it('keeps the latest refresh content when an older refresh resolves late', async t => {
    ...
    let provider: TextDocumentContentProvider = {
      ...
      provideTextDocumentContent: () => {
        ...
        if (calls === 2) return new Promise<string>(resolve => {
          first = resolve
        })
    ...
    try {
      ...
      second('new content')
      first('stale content')
      await session.waitFor('getline', ['.'], 'new content')
  ...
  it('does not write the buffer after the provider is unregistered', async t => {
...
async function triggerAutocmd(id: number): Promise<void> {
  await session.plugin.cocAction('doAutocmd', id)
}

editorSuite('setupDynamicAutocmd()', () => {
  afterEach(() => {
  ...
  it('should convert to autocmd option', t => {
  ...
  it('should setup autocmd', async t => {
    await nvim.setLine('foo')
    let times = 0
  ...
  it('should remove autocmd from nvim on dispose', async t => {
  ...
  it('should remove user autocmd from nvim on dispose', async t => {
  ...
  it('should keep same event autocmd after other is disposed', async t => {
    let first = 0
    let second = 0
    let disposable = workspace.registerAutocmd({
      event: 'CursorMoved',
  ...
  it('should not throw on autocmd callback error', async t => {
    let called = false
    let disposable = workspace.registerAutocmd({
      event: 'CursorHold',
      request: false,
  ...
  it('should not throw on rejecting autocmd callback', async t => {
    let called = 0
    let disposable = workspace.registerAutocmd({
      event: 'CursorHold',
      request: true,
  ...
  it('should setup user autocmd', async t => {
    let called = false
    workspace.registerAutocmd({
...
editorSuite('doAutocmd()', () => {
  it('should not throw when command id does not exist', async t => {
    await workspace.autocmds.doAutocmd(999, [])
  })

  it('should cancel timeout request autocmd', async t => {
    let cancelled = false
    ...
    let max = Math.max(...Array.from(keys) as number[])
...
editorSuite('create terminal', () => {
  it('should use cleaned env', async t => {
    ...
    await session.waitFor('eval', [`join(getbufline(${terminal.bufnr},1,'$'),'\n')`], /\S/)
    let lines = await buf.lines
  ...
  it('strictEnv forwards caller-provided variables to terminal startup', async t => {
    ...
    let startCall = spy.mock.calls.find(c => c.arguments[0] === 'coc#terminal#start')
    assert.ok(startCall)
    let startArgs = startCall.arguments[1] as any[]
    assert.deepStrictEqual(startArgs[0], [which.sync('bash')])
    assert.strictEqual(typeof startArgs[1], 'string')
    assert.deepStrictEqual(startArgs[2], { COC_AUDIT_ENV: 'wanted' })
    assert.strictEqual(startArgs[3], true)
  })

  it('restores editor state when terminal start fails', async t => {
  ...
  it('should use custom shell command', async t => {
    let terminal = await terminals.createTerminal(nvim, {
      name: `test-${crypto.randomUUID()}`,
      shellPath: which.sync('bash')
    })
  ...
  it('should use custom cwd', async t => {
  ...
  it('should have exit code', async t => {
    ...
    await session.waitFor('bufloaded', [terminal.bufnr], 0)
  ...
  it('cleans the terminal channel map after exit and dispose', async t => {
    ...
    await session.waitFor('bufloaded', [t1.bufnr], 0)
    ...
    await session.waitFor('bufloaded', [t2.bufnr], 0)
    assert.strictEqual(await nvim.call('coc#terminal#_channel_count'), base)
    // manual dispose
    ...
    t3.dispose()
    await session.waitFor('bufloaded', [t3.bufnr], 0)
    assert.strictEqual(await nvim.call('coc#terminal#_channel_count'), base)
  })
  ...
  it('should show terminal on current window', async t => {
    let terminal = await terminals.createTerminal(nvim, {
      name: `test-${crypto.randomUUID()}`,
      shellPath: which.sync('bash')
    })
    let winid = await nvim.call('bufwinid', [terminal.bufnr]) as number
    assert.ok(winid > 0)
    await nvim.call('win_gotoid', [winid])
    await terminal.show()
  })

  it('should show terminal that shown', async t => {
    let terminal = await terminals.createTerminal(nvim, {
      name: `test-${crypto.randomUUID()}`,
      shellPath: which.sync('bash')
    })
  ...
  it('should show hidden terminal', async t => {
    let terminal = await terminals.createTerminal(nvim, {
      name: `test-${crypto.randomUUID()}`,
      shellPath: which.sync('bash')
    })
  ...
  it('should create terminal', async t => {
    ...
    terminal.dispose()
    await session.waitValue(() => terminal.bufnr, undefined)
