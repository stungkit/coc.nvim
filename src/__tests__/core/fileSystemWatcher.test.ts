...
before(() => new Promise<void>(done => {
  ...
  watcherManager.attach(session.createNullChannel())
...
editorSuite('FileSystemWatcherManager.disabled', () => {
...
editorSuite('Watchman#subscribe', () => {

  it('should subscribe file change', async t => {
    let client = new Watchman(null, session.createNullChannel())
    disposables.push(client)
    await client.watchProject(cwd)
    let called = false
  ...
  it('should not subscribe invalid response', async t => {
    let c = new Watchman(null, session.createNullChannel())
    disposables.push(c)
    watchResponse = { watch: cwd, relative_path: 'foo' }
...
editorSuite('Watchman#createClient', () => {
...
editorSuite('create FileSystemWatcherManager', () => {
  ...
  it('disposes a client whose creation completes after the folder was removed', async t => {
    ...
    await session.waitValue(() => createSpy.mock.calls.length, 1)
...
editorSuite('FileSystemWatcher dispose', () => {
