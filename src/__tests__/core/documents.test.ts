...
editorSuite('documents', () => {
  ...
  it('should get document', async t => {
    await session.createDocument('bar')
    let doc = await session.createDocument('foo')
  ...
  it('should resolveRoot', async t => {
    ...
    await session.edit(import.meta.filename)
  ...
  it('should get languageId', async t => {
    await session.createDocument('t.vim')
  ...
  it('should get lines', async t => {
    let doc = await session.createDocument('tmp')
  ...
  it('should get QuickfixItem from location link', async t => {
    let doc = await session.createDocument('quickfix')
  ...
  it('should try code actions', async t => {
    session.updateConfiguration('editor.codeActionsOnSave', { 'source.fixAll': false }, disposables)
    let doc = await workspace.document
    let res = await documents.tryCodeActionsOnSave(doc)
    assert.strictEqual(res, false)
    session.updateConfiguration('editor.codeActionsOnSave', {
...
editorSuite('formatOnSave', () => {
  it('should not throw when provider not found', async t => {
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['javascript'], disposables)
    let filepath = await createTmpFile('')
    await session.edit(filepath)
  ...
  it('should invoke format on save', async t => {
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['text'], disposables)
    ...
    let buf = await session.edit(filepath)
  ...
  it('should cancel when timeout', async t => {
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['*'], disposables)
    session.updateConfiguration('coc.preferences.formatOnSaveTimeout', 100, disposables)
    let timer
  ...
  it('should enable format on save', async t => {
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', null)
    let doc = await workspace.document

    session.updateConfiguration('coc.preferences.formatOnSave', false)
    let res = documents.shouldFormatOnSave(doc)
    assert.strictEqual(res, false)

    session.updateConfiguration('coc.preferences.formatOnSave', true)
  ...
  it('should prefer formatOnSaveFiletypes over formatOnSave', async t => {
    ...
    session.updateConfiguration('coc.preferences.formatOnSave', true, disposables)
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['javascript'], disposables)
    ...
    session.updateConfiguration('coc.preferences.formatOnSave', false, disposables)
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['text'], disposables)
  ...
  it('should ignore non-array formatOnSaveFiletypes', async t => {
    ...
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', 123, disposables)
    assert.strictEqual(documents.shouldFormatOnSave(doc), false)
  })

  it('should not format on save when disabled', async t => {
    session.updateConfiguration('coc.preferences.formatOnSaveFiletypes', ['text'])
    disposables.push(languages.registerDocumentFormatProvider(['text'], {
      provideDocumentFormattingEdits: document => {
        let lines = document.getText().replace(/\n$/, '').split(/\n/)
        let edits: TextEdit[] = []
