...
autocmd User CocLocationsChange :call OnLocationsChange()`)
    let locations = createLocations()
    await workspace.showLocations(locations)
    await session.waitFor('eval', [`get(g:,'called',0)`], 1)
  })

  it('should show quickfix when quickfix enabled', async t => {
    session.updateConfiguration('coc.preferences.useQuickfixForLocations', true)
    let locations = createLocations()
    await workspace.showLocations(locations)
    await session.waitFor('eval', [`&buftype`], 'quickfix')
  })
  ...
  it('should use customized quickfix open command', async t => {
    ...
    await nvim.command('unlet! g:coc_quickfix_open_command', true)
  })
})
...
editorSuite('openResource()', () => {
  ...
  it('should open opened buffer', async t => {
    ...
    await session.waitFor('bufnr', ['%'], buf.id)
  })
...
editorSuite('WorkspaceFolderController', () => {
