...
editorSuite('services', () => {
  describe('functions', () => {
    ...
    it('should use socket port for language server #2', async t => {
      ...
      let fn = opts[1] as Function
      let res = await fn()
      await session.waitValue(() => connected, true)
  ...
  describe('getServiceStats()', () => {
    it('should get services', async t => {
      let res = await session.doAction('services')
      assert.notStrictEqual(res, undefined)
    })
  })
  ...
  describe('toggle()', () => {
    it('should throw when service not found', async t => {
      let fn = async () => {
        await session.doAction('toggleService', 'id')
      }
      await assert.rejects(fn(), Error)
    })
  ...
  describe('start()', () => {
    ...
    it('should start language client on by document', async t => {
      ...
      await session.doAction('registerNotification', 'test', 'notification')
  ...
  describe('registerLanguageClient', () => {
    ...
    it('should sendRequest & sendNotification', async t => {
      ...
      let service = services.getService('test')
      await service.start()
      let res = await session.plugin.cocAction('sendRequest', 'test', 'request', { value: 2 })
      assert.strictEqual(res, 3)
      await session.plugin.cocAction('sendNotification', 'test', 'notification', {})
    ...
    it('should register notification when client created', async t => {
      ...
      let service = services.getService('test')
      await session.plugin.cocAction('registerNotification', 'test', 'notification')
      await service.start()
    ...
    it('should register notification when client not created', async t => {
      await session.plugin.cocAction('registerNotification', 'def', 'notification')
      workspace.configurations.updateMemoryConfig({
        languageserver: {
          def: {
            filetypes: ['vim'],
