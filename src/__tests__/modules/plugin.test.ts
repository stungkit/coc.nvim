...
editorSuite('typings declarations', () => {
...
editorSuite('help tags', () => {
  it('should return jumpable', async t => {
    let jumpable = await session.plugin.cocAction('snippetCheck', false, true)
    assert.strictEqual(jumpable, false)
  })

  it('should show CocInfo', async t => {
    await session.doAction('showInfo')
  ...
  it('should ensure current document created', async t => {
    ...
    let res = await session.plugin.cocAction('ensureDocument')
  ...
  it('should get related information', async t => {
    let res = await session.plugin.cocAction('diagnosticRelatedInformation')
