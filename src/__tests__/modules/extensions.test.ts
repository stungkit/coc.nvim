...
editorSuite('extensions', () => {
  ...
  it('should has extension', async t => {
    ...
    let loaded = await session.doAction('loadedExtensions')
    assert.deepStrictEqual(loaded, [])
    let stats = await session.doAction('extensionStats')
  ...
  it('should catch error on updateExtensions', async t => {
    ...
    await session.doAction('updateExtensions', true)
  })
  ...
  it('should install global extension', async t => {
    ...
    await session.doAction('installExtensions', 'coc-omni')
    ...
    await session.doAction('reloadExtension', 'coc-omni')
    await session.doAction('deactivateExtension', 'coc-omni')
    await session.doAction('activeExtension', 'coc-omni')
    await session.doAction('toggleExtension', 'coc-omni')
    await session.doAction('uninstallExtension', 'coc-omni')
