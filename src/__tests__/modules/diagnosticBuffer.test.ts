...
editorSuite('diagnostic buffer', () => {
  describe('showFloat()', () => {
    ...
    it('should show formated diagnostics', async t => {
      session.updateConfiguration('diagnostic.format', '[%source] %message')
      let buf = await createDiagnosticBuffer()
  ...
  describe('refresh()', () => {
    it('should not add signs when disabled', async t => {
      session.updateConfiguration('diagnostic.enableSign', false)
    ...
    it('should filter sign by signLevel', async t => {
      session.updateConfiguration('diagnostic.signLevel', 'error')
  ...
  describe('setDiagnosticInfo()', () => {
    it('should include lines', async t => {
      session.updateConfiguration('diagnostic.virtualTextCurrentLineOnly', false)
      let buf = await createDiagnosticBuffer()
  ...
  describe('showVirtualText()', () => {
    ...
    it('should change format of virtualText message', async t => {
      session.updateConfiguration('diagnostic.virtualTextFormat', '%source %message')
      let buf = await createDiagnosticBuffer()
      let diagnostic = createDiagnostic('foo')
    ...
    it('should show virtual text at window column', async t => {
      session.updateConfiguration('diagnostic.virtualTextWinCol', 90)
      let diagnostic = createDiagnostic('foo')
      let buf = await createDiagnosticBuffer()
      let diagnostics = [diagnostic]
      await buf.update('', diagnostics)
    ...
    it('should filter by virtualTextLevel', async t => {
      session.updateConfiguration('diagnostic.virtualTextLevel', 'error')
      session.updateConfiguration('diagnostic.virtualTextAlign', 'after')
      let buf = await createDiagnosticBuffer()
      let diagnostics = [
        createDiagnostic('foo', Range.create(0, 0, 0, 1), DiagnosticSeverity.Error),
    ...
    it('should limit virtual text count of one line', async t => {
      session.updateConfiguration('diagnostic.virtualTextCurrentLineOnly', false)
      session.updateConfiguration('diagnostic.virtualTextLimitInOneLine', 1)
      let buf = await createDiagnosticBuffer()
      let diagnostics = [
        createDiagnostic('foo', Range.create(0, 0, 0, 1)),
  ...
  describe('updateLocationList()', () => {
    beforeEach(() => {
      session.updateConfiguration('diagnostic.locationlistUpdate', true)
    })
  ...
  describe('reset()', () => {
    it('should clear exists diagnostics', async t => {
      ...
      await session.waitValue(async () => (await buf.doc.buffer.getVar('coc_diagnostic_info') as any)?.error, 1)
      await buf.reset({})
