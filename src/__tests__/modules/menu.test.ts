...
editorSuite('Menu', () => {
  ...
  it('should ignore invalid index', async t => {
    ...
    await session.waitPrompt()
    await nvim.input('0')
    await session.waitValue(() => nvim.call('coc#float#has_float', []), 1)
  ...
  it('should navigate by j, k, g & G', async t => {
    ...
    await nvim.input('j')
    await nvim.input('j')
    await session.waitValue(async () => (await win.cursor)[0], 1)
    ...
    assert.strictEqual(cursor[0], 1)
    await nvim.input('G')
    await session.waitValue(async () => (await win.cursor)[0], 3)
    cursor = await win.cursor
