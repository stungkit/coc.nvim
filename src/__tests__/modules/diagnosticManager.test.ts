...
async function createDocument(name?: string): Promise<Document> {
  let doc = await session.createDocument(name)
...
editorSuite('diagnostic manager', () => {
  afterEach(() => {
    manager.reset()
  })
  ...
  describe('setLocationlist()', () => {
    it('should set location list', async t => {
      ...
      await session.doAction('fillDiagnostics', doc.bufnr)
      let res = await nvim.call('getloclist', [doc.bufnr]) as any[]
      assert.ok(res.length > 2)
      session.updateConfiguration('diagnostic.locationlistLevel', 'error')
  ...
  describe('events', () => {
    it('should delay refresh when buffer visible', async t => {
      ...
      await session.waitFor('eval', ['empty(get(b:,"coc_diagnostic_info",{}))'], 0)
      collection.dispose()
    })
    ...
    it('should show diagnostic virtual text on CursorMoved', async t => {
      session.updateConfiguration('diagnostic.virtualText', true)
      session.updateConfiguration('diagnostic.virtualTextCurrentLineOnly', true)
      let doc = await createDocument()
      await session.waitValue(async () => (await doc.buffer.getExtMarks(virtualTextSrcId, 0, -1, { details: true })).length > 0, true)
      ...
      await session.waitValue(async () => (await doc.buffer.getExtMarks(virtualTextSrcId, 0, -1, { details: true })).length, 0)
  ...
  describe('refresh()', () => {
    it('should refresh on buffer create', async t => {
      ...
      // from: EditorSession resolves relative names against the nvim cwd
      // (build tree), while import.meta.dirname here points at the source tree.
      let doc = await session.createDocument(path.join(path.dirname(import.meta.dirname), 'doc'))
      await session.waitValue(() => fn.mock.calls.length, 1)
  ...
  describe('toggleDiagnostic()', () => {
    it('should toggle diagnostics for all buffer', async t => {
      ...
      let doc = await createDocument()
      await session.doAction('diagnosticToggle')
  ...
  describe('getDiagnosticList()', () => {
    it('should get all diagnostics', async t => {
      ...
      let doc = await session.createDocument(fsPath)
      let diagnostics: Diagnostic[] = []
      ...
      let list = await session.doAction('diagnosticList')
    ...
    it('should filter diagnostics by configuration', async t => {
      session.updateConfiguration('diagnostic.level', 'warning')
      session.updateConfiguration('diagnostic.showUnused', false)
      session.updateConfiguration('diagnostic.showDeprecated', false)
      let doc = await createDocument()
      let buf = manager.getItem(doc.bufnr)
  ...
  describe('preview()', () => {
    it('should not throw with empty diagnostics', async t => {
      await session.doAction('diagnosticPreview')
  ...
  describe('getDiagnosticsInRange', () => {
    it('should get diagnostics in range', async t => {
      ...
      doc = await session.createDocument()
  ...
  describe('getCurrentDiagnostics', () => {
    it('should get undefined when buffer not attached', async t => {
      ...
      await session.doAction('diagnosticInfo')
    ...
    it('should get diagnostics under cursor', async t => {
      ...
      session.updateConfiguration('diagnostic.checkCurrentLine', true)
  ...
  describe('jumpRelated', () => {
    ...
    it('should open location list', async t => {
      ...
      await nvim.call('cursor', [1, 1])
      await manager.jumpRelated()
      await session.waitFor('bufname', ['%'], 'list:///location')
      await nvim.input('<esc>')
    })
  })
  ...
  describe('jumpPrevious & jumpNext', () => {
    it('should jump to previous', async t => {
      ...
      await session.doAction('diagnosticPrevious')
    })
    ...
    it('should jump to next', async t => {
      ...
      await session.doAction('diagnosticNext')
    })

    it('should consider invalid position', async t => {
      let doc = await session.createDocument('foo.js')
      let collection = manager.create('foo')
      let diagnostics: Diagnostic[] = []
  ...
  describe('diagnostic configuration', () => {
    it('should use filetype map from config', async t => {
      session.updateConfiguration('diagnostic.filetypeMap', { default: 'bufferType' })
      session.updateConfiguration('diagnostic.messageDelay', 10)
    ...
    it('should show floating window on cursor hold', async t => {
      session.updateConfiguration('diagnostic.messageTarget', 'float')
      session.updateConfiguration('diagnostic.messageDelay', 10)
      await createDocument()
      await nvim.call('cursor', [1, 3])
      let winid = await session.waitFloat()
    ...
    it('should filter diagnostics by messageLevel', async t => {
      session.updateConfiguration('diagnostic.messageLevel', 'error')
      session.updateConfiguration('diagnostic.messageTarget', 'echo')
      await createDocument()
    ...
    it('should not echo messages on CursorHold', async t => {
      ...
      session.updateConfiguration('diagnostic.enableMessage', 'never')
      await createDocument()
      await nvim.call('cursor', [1, 3])
    ...
    it('should send ale diagnostic items', async t => {
      session.updateConfiguration('diagnostic.displayByAle', true)
      let content = `
    ...
    `
      ...
      await session.waitFor('eval', ['get(g:,"items",[])'], [])
    })

    it('should send to vim.diagnostic', async t => {
      session.updateConfiguration('diagnostic.displayByVimDiagnostic', true)
  ...
  describe('toggleDiagnosticBuffer', () => {
    it('should not throw when bufnr is invliad or disabled', async t => {
      let doc = await workspace.document
      await session.doAction('diagnosticToggleBuffer', 99)
      session.updateConfiguration('diagnostic.enable', false)
      await manager.toggleDiagnosticBuffer(doc.bufnr)
    })
  ...
  describe('refresh', () => {
    beforeEach(() => {
      session.updateConfiguration('diagnostic.autoRefresh', false)
    })
    ...
    it('should refresh all buffers', async t => {
      ...
      await session.doAction('diagnosticRefresh')
