...
editorSuite('workspace properties', () => {
  it('should have initialized', async t => {
    ...
    let rootPath = await session.doAction('currentWorkspacePath')
    assert.strictEqual(rootPath, process.cwd())
  })

  it('should get filetyps', async t => {
    await session.edit('f.js')
...
editorSuite('workspace methods', () => {
  ...
  it('should expand variables', async t => {
    ...
    await session.edit(path.join(path.dirname(import.meta.dirname), 'bar.ts'))
  ...
  it('should return match score for document', async t => {
    let doc = await session.createDocument('tmp.xml')
  ...
  it('should getWatchConfig', async t => {
    session.updateConfiguration('fileSystemWatch.enable', null, disposables)
    session.updateConfiguration('fileSystemWatch.watchmanPath', '~/bin/watchman', disposables)
    session.updateConfiguration('fileSystemWatch.ignoredFolders', ['~'], disposables)
...
editorSuite('workspace utility', () => {
  ...
  it('should findUp to tsconfig.json from current file', async t => {
    await session.edit(path.join(import.meta.dirname, 'edit'))
  ...
  it('should findUp from current file', async t => {
    ...
    await session.edit(path.join(process.cwd(), 'foo'))
    let filepath = await workspace.findUp('tsconfig.json')
    assert.match(filepath, new RegExp('tsconfig\.json'))
  })
  ...
  it('should register autocmd', async t => {
    ...
    await session.waitValue(() => eventCount, 1)
  ...
  it('should resolve dynamic insert keymaps against current state', async t => {
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        await session.waitValue(async () => await nvim.getLine(), "('')")
        outputs.push(await nvim.getLine())
      }
      assert.deepStrictEqual([...new Set(outputs)], ["('')"])
      await nvim.command('enew!')
      await nvim.command('normal Oa[')
      await session.waitValue(async () => await nvim.getLine(), 'a[]')
...
editorSuite('workspace events', () => {

  it('should listen to fileType change', async t => {
    let buf = await session.edit()
    await nvim.command('setf xml')
    await session.waitValue(() => workspace.getDocument(buf.id)?.filetype, 'xml')
    let doc = workspace.getDocument(buf.id)
  ...
  it('should fire onWillSaveUntil', async t => {
    ...
    await session.waitValue(() => doc.getDocumentContent().startsWith('foobar'), true)
  ...
  it('should only use first returned textEdits', async t => {
    ...
    await doc.synchronize()
    await nvim.command('wa')
    await session.waitValue(() => doc.getDocumentContent().includes('foo'), true)
    let content = doc.getDocumentContent()
...
editorSuite('workspace registerBufferSync', () => {
