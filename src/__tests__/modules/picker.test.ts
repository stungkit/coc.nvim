...
editorSuite('Picker key mappings', () => {
  it('should toggle selection mouse click bracket', async t => {
    ...
    await session.waitValue(async () => (await buf.getLines({ start: 0, end: 1, strictIndexing: false }))[0].startsWith('[x]'), true)
  ...
  it('should move cursor by j, k, g & G', async t => {
    ...
    assert.notStrictEqual(winid, undefined)
    await nvim.input('j')
    await session.waitValue(async () => (await getSigns())[0]?.signs?.[0]?.lnum, 2)
    let res = await getSigns()
    assert.strictEqual(res[0].signs[0].lnum, 2)
    await nvim.input('k')
    await session.waitValue(async () => (await getSigns())[0]?.signs?.[0]?.lnum, 1)
    res = await getSigns()
