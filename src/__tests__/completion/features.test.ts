...
async function waitFloat(kind: string): Promise<void> {
  for (let i = 0; i < 50; i++) {
    ...
    let win = await session.getFloat(kind)
    if (win) return
  }
...
editorSuite('completion float', () => {
  ...
  it('should cancel float window', async t => {
    ...
    await session.waitFor('coc#float#has_float', [], 0)
  })
  ...
  it('should adjust float window position', async t => {
    ...
    await nvim.input('Af')
    await session.visible('foo', 'float')
    await waitFloat('pumdetail')
  ...
  it('should redraw float window on item change', async t => {
    ...
    try {
      ...
      await session.waitValue(() => mock.createCalls.length > 0, true)
  ...
  it('should hide float window when item info is empty', async t => {
    ...
    try {
      ...
      await session.waitValue(() => mock.closeCalls > 0, true)
...
editorSuite('float config', () => {
  ...
  async function getFloat(): Promise<number> {
    let win = await session.getFloat('pumdetail')
    return win ? win.id : -1
  }
...
editorSuite('Source', () => {
  ...
  it('should check trigger only source', async t => {
    ...
    session.updateConfiguration(`coc.source.${name}.triggerPatterns`, [null, 'foo'])
    assert.strictEqual(s.triggerOnly, true)
  })
  ...
  it('should check complete', async t => {
    ...
    session.updateConfiguration(`coc.source.${name}.disableSyntaxes`, ['comment'])
    await nvim.input('i')
    let opt = await nvim.call('coc#util#get_complete_option') as CompleteOption
    opt.synname = 'Comment'
...
function! coc#source#issue5539#complete(opt, cb) abort
  call a:cb(['Microsoft Windows'])
    ...
    assert.strictEqual(session.completion.activeItems.some(item => item.word == 'Microsoft Windows'), true)
  })
  ...
  it('should not run by check complete', async t => {
    ...
    session.updateConfiguration('coc.source.vim.disableSyntaxes', ['comment'])
    session.updateConfiguration('coc.source.vim.filetypes', ['vim'])
...
editorSuite('native sources', () => {
  ...
  it('should works for around source', async t => {
    ...
    await nvim.input('Af')
    await session.waitPopup()
    let res = await session.visible('foo', 'around')
    assert.strictEqual(res, true)
  ...
  it('should works for buffer source', async t => {
    ...
    assert.strictEqual(mode, 'n')
    await nvim.input('io')
    let res = await session.visible('other', 'buffer')
    assert.strictEqual(res, true)
  })
  ...
  it('should trim ext for file source', async t => {
    ...
    nvim.call('coc#start', { source: 'file' }, true)
    await session.waitPopup()
    let items = session.completion.activeItems
