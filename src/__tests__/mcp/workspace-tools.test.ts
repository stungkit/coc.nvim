...
before(() => {
  ...
  realCommand = session.nvim.command.bind(session.nvim)
  // Suite-level before hooks have no per-test MockTracker (t.mock); use the
  // module-level tracker and restore it in after().
  commandSpy = mock.method(session.nvim, 'command', ((command: string, isNotify?: boolean) => {
    if (command === 'wa') {
...
beforeEach(() => {
  // editorSuite resets the workspace folder control after each test;
...
editorSuite('mcp workspace tools', () => {
