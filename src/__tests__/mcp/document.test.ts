...
afterEach((t: any) => {
  ...
  disposables = []
  // The outer editor reset performs RPC calls, so restore per-test RPC
  // mocks before that reset starts.
  t.mock.restoreAll()
})
...
describe('mcp document tools', () => {
  it('document/read returns unsaved buffer changes', async t => {
    await session.nvim.call('setline', [2, 'changed-line'])
  ...
  it('document/apply_edits updates the buffer and fires didChange', async t => {
    ...
    let lines = await session.nvim.call('getline', [1, '$']) as string[]
  ...
  it('edits can be reverted with undo', async t => {
    let before = ((await session.nvim.call('getline', [1, '$'])) as string[]).join('\n')
    let result = await tool('document/apply_edits').handler({
      uri: file,
      target: 'buffer',
      joinUndo: false,
    ...
    assert.ok(!result.isError)
    let after = ((await session.nvim.call('getline', [1, '$'])) as string[]).join('\n')
    assert.notStrictEqual(after, before)
    await session.nvim.command('undo')
    let reverted = ((await session.nvim.call('getline', [1, '$'])) as string[]).join('\n')
    assert.strictEqual(reverted, before)
  })

  it('document/write saves the buffer to disk', async t => {
    await session.nvim.call('setline', [1, 'saved-line'])
  ...
  it('document/open loads a file into a buffer', async t => {
    ...
    assert.ok(!result.isError)
    await session.waitValue(() => session.nvim.call('bufloaded', [other]), 1)
  })
  ...
  it('document/open opens multiple files at the requested lines', async t => {
    ...
    await session.waitValue(() => session.nvim.call('bufloaded', [f1]), 1)
    await session.waitValue(() => session.nvim.call('bufloaded', [f2]), 1)
    // the last opened file is current, cursor at the requested 1-based line
    let lnum = await session.nvim.eval('line(".")') as number
    assert.strictEqual(lnum, 3)
    let lineText = await session.nvim.call('getline', ['.']) as string
  ...
  it('document/open validates targets and supports line and column fragments', async t => {
    ...
    assert.ok(!result.isError)
    assert.deepStrictEqual(await session.nvim.eval('[line("."), col(".")]'), [2, 2])
  })
