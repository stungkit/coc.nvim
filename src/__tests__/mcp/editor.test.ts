...
beforeEach(async () => {
  ...
  await session.nvim.command('setfiletype typescript')
  await session.waitValue(() => !!workspace.getDocument(uri), true)
  await session.waitValue(() => workspace.getDocument(uri)!.languageId, 'typescript')
})
...
describe('mcp editor tools', () => {
  ...
  it('editor/state returns active editor state', async t => {
    await session.nvim.call('cursor', [2, 3])
  ...
  it('editor/state returns visual selection', async t => {
    await session.nvim.call('cursor', [2, 1])
    await session.nvim.input('v$')
    let result = await tool().handler({}, { token })
    await session.nvim.input('<esc>')
    let s = result.structuredContent
  ...
  it('editor/state normalizes a backwards visual selection', async t => {
    await session.nvim.call('cursor', [2, 8])
    await session.nvim.input('v0')
    let result = await tool().handler({}, { token })
    await session.nvim.input('<esc>')
  ...
  it('editor/state returns null selection outside visual mode', async t => {
    await session.nvim.input('v<esc>')
    let result = await tool().handler({}, { token })
