...
editorSuite('BackgroundScheduler', () => {
...
editorSuite('DocumentPullStateTracker', () => {
...
editorSuite('DiagnosticRequestor', () => {
  ...
  it('should not reschedule workspace pull after dispose', async t => {
    ...
    await session.waitValue(() => calls, 1)
    manager.dispose()
    resolvePull({ items: [] })
    await session.waitValue(() => calls, 1)
  ...
  it('should reset workspace error counter on success', async t => {
    ...
    await session.waitValue(() => calls, 10)
...
editorSuite('DiagnosticFeature', () => {
  ...
  it('should filter by document selector', async t => {
    ...
    await session.waitValue(() => window.activeTextEditor?.uri === doc.uri, true)
