...
editorSuite('Outline util', () => {
  ...
  it('should load items by ctags', async t => {
    ...
    doc = await session.createDocument(import.meta.filename)
...
editorSuite('list worker', () => {
  ...
  it('should sort by sortText', async t => {
    ...
    await session.waitFor('getline', ['.'], 'ade')
    await manager.cancel()
  })
  ...
  it('should render slow interactive list', async t => {
    ...
    await session.listInput('a')
    await session.waitFor('getline', [2], 'abort')
  })
  ...
  it('should work with interactive list', async t => {
    ...
    await session.waitFor('getline', ['.'], 'fax')
    await manager.cancel(true)
  })
...
editorSuite('list session', () => {
  ...
  describe('resume()', () => {
    it('should do preview on resume', async t => {
      ...
      await session.waitValue(() => lastItem != null, true)
  ...
  describe('jumpBack()', () => {
    it('should jump back', async t => {
      ...
      await session.waitValue(() => nvim.call('win_getid'), win.id)
...
editorSuite('chooseAction()', () => {
  ...
  it('should choose action by menu picker', async t => {
    session.updateConfiguration('list.menuAction', true)
    labels = ['a', 'b', 'c']
    let fn = t.mock.fn()
    let list = new SimpleList()
    let len = list.actions.length
...
editorSuite('Command task', () => {
  ...
  it('should not show error', async t => {
    ...
    await session.waitValue(() => manager.session?.ui.length, 0)
  ...
  it('should create command task', async t => {
    ...
    await manager.start(['data'])
    await manager.session.ui.ready
    await session.waitValue(async () => nvim.call('getline', [1, '$']), ['foo', 'bar'])
    let lines = await nvim.call('getline', [1, '$']) as string[]
