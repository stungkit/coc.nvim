...
editorSuite('configuration', () => {
  ...
  it('should change default options', async t => {
    session.updateConfiguration('list.source.option.defaultOptions', ['--normal'])
  ...
  it('should change default action', async t => {
    session.updateConfiguration('list.source.option.defaultAction', 'split')
    await manager.start(['option'])
    await manager.session.ui.ready
  ...
  it('should change default arguments', async t => {
    session.updateConfiguration('list.source.option.defaultArgs', ['-word'])
    await manager.start(['option'])
    await manager.session.ui.ready
...
editorSuite('list sources', () => {
  ...
  describe('diagnostics', () => {
    ...
    async function createDocument(name?: string): Promise<Document> {
      let doc = await session.createDocument(name ? path.join(import.meta.dirname, '..', name) : name)
  ...
  describe('extensions', () => {
    it('should load extensions source', async t => {
      ...
      await doAction('configuration', items[1])
      await session.mockFunction('coc#ui#open_url', 0)
  ...
  describe('folders', () => {
    it('should load folders source', async t => {
      await session.createDocument(import.meta.filename)
  ...
  describe('outline', () => {
    ...
    it('should load items by ctags', async t => {
      session.updateConfiguration('list.source.outline.ctagsFiletypes', ['vim'])
  ...
  describe('sources', () => {
    it('should load sources source', async t => {
      ...
      let bufname = await nvim.call('bufname', '%') as string
      assert.match(bufname, /(?:native|bundle\.js|coc-bundle-runtime)/)
    })
  ...
  describe('notifications', () => {
    ...
    it('should load notifications from action', async t => {
      ...
      const res = await session.doAction('notificationHistory')
      assert.strictEqual(res.length, 1)
  ...
  describe('symbols', () => {
    ...
    it('should load items', async t => {
      ...
      assert.deepStrictEqual(res, [])
      context.args = []
      session.updateConfiguration('list.source.symbols.excludes', ['**/*.ts'])
      res = await source.loadItems(context, CancellationToken.None)
      assert.deepStrictEqual(res, [])
      session.updateConfiguration('list.source.symbols.excludes', [])
      res = await source.loadItems(context, CancellationToken.None)
      assert.strictEqual(res.length, 1)
    })
