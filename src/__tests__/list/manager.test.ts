...
async function waitCmdline(text: string): Promise<void> {
  await session.waitValue(async () => (await session.getCmdline()).includes(text), true)
}
...
editorSuite('list', () => {
  ...
  describe('list configuration', () => {
    it('should change indicator', async t => {
      session.updateConfiguration('list.indicator', '>>')
    ...
    it('should split right for preview window', async t => {
      session.updateConfiguration('list.previewSplitRight', true)
    ...
    it('should use smartcase for strict match', async t => {
      session.updateConfiguration('list.smartCase', true)
    ...
    it('should change next and previous keymap', async t => {
      session.updateConfiguration('list.nextKeymap', '<tab>')
      session.updateConfiguration('list.previousKeymap', '<s-tab>')
      await manager.start(['location'])
      await manager.session.ui.ready
      await session.waitPrompt()
    ...
    it('should toggle preview', async t => {
      session.updateConfiguration('list.floatPreview', true)
  ...
  describe('descriptions', () => {
    it('should get descriptions', async t => {
      let res = await session.doAction('listDescriptions')
  ...
  describe('loadItems()', () => {
    ...
    it('should load items for list', async t => {
      ...
      manager.toggleMode()
      manager.stop()
      res = await session.doAction('listLoadItems', '')
  ...
  describe('onInsertInput()', () => {
    ...
    it('reports interactive reload errors and keeps the worker usable', async t => {
      ...
      try {
        ...
        manager.session.onInputChange()
        await session.waitValue(() => calls, 2)
        await session.waitValue(() => manager.session.worker.isLoading, false)
        ...
        manager.session.onInputChange()
        await session.waitValue(() => calls, 3)
        await session.waitValue(() => manager.session.worker.isLoading, false)
        assert.strictEqual(manager.session.worker.isLoading, false)
      } finally {
  ...
  describe('resume()', () => {
    it('should resume by name', async t => {
      ...
      await session.doAction('listResume')
      assert.strictEqual(manager.isActivated, true)
  ...
  describe('first(), last()', () => {
    it('should get session by name', async t => {
      ...
      await manager.session.ui.ready
      await session.doAction('listFirst', 'a')
      await session.doAction('listLast', 'a')
  ...
  describe('registerList()', () => {
    it('should recreate list', async t => {
      ...
      manager.registerList(list, true)
      session.updateConfiguration('list.source.test.defaultAction', 'open')
      let disposable = manager.registerList(list, true)
      disposable.dispose()
