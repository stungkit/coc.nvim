...
async function waitPreviewWindow(): Promise<void> {
  await session.waitValue(() => nvim.call('coc#list#has_preview').then(n => (n as number) > 0), true)
}
...
editorSuite('User mappings', () => {
  ...
  it('should show warning for invalid key', async t => {
    ...
    let revert = session.updateConfiguration('list.insertMappings', {
      xy: 'action:tabe',
    })
    ...
    revert = session.updateConfiguration('list.insertMappings', {
      '<M-x>': 'action:tabe',
    })
    assert.ok(warningSpy.mock.calls.some(c => typeof c.arguments[0] === 'string' && c.arguments[0].includes('Invalid configuration')))
    revert()
  ...
  it('should execute expr keymap', async t => {
    installSpies(t)
    await session.mockFunction('TabOpen', 'quickfix')
    session.updateConfiguration('list.insertMappings', {
      '<C-t>': 'expr:TabOpen',
    })
    session.updateConfiguration('list.normalMappings', {
      t: 'expr:TabOpen',
    })
    await manager.start(['location'])
    await manager.session.ui.ready
    await session.listInput('<C-t>')
  ...
  it('should execute do mappings', async t => {
    session.updateConfiguration('list.previousKeymap', '<C-j>')
    session.updateConfiguration('list.nextKeymap', '<C-k>')
    session.updateConfiguration('list.insertMappings', {
      '<C-n>': 'do:next',
      '<C-p>': 'do:previous',
      '<C-d>': 'do:exit',
  ...
  it('should execute call keymap', async t => {
    await session.mockFunction('Test', 1)
    session.updateConfiguration('list.insertMappings', {
      '<C-t>': 'call:Test',
    })
    await manager.start(['location'])
  ...
  it('should insert text from default register to prompt', async t => {
    ...
    await manager.start(['location'])
    await manager.session.ui.ready
    await session.listInput('<C-v>')
    let { input } = manager.prompt
    assert.match(input, new RegExp('bar'))
  })
})

editorSuite('doAction()', () => {
  ...
  it('should not throw when action name does not exist', async t => {
    await session.mockFunction('MyExpr', '')
    let mappings = manager.mappings
    await mappings.doAction('expr', 'MyExpr')
  })
})

editorSuite('getAction()', () => {
...
editorSuite('Default normal mappings', () => {
  ...
  it('should stop task by <C-c>', async t => {
    ...
    await session.waitValue(() => manager.session?.worker.isLoading, true)
...
editorSuite('list insert mappings', () => {
  ...
  it('should select action by insert <tab>', async t => {
    ...
    await session.listInput('<tab>')
    assert.ok(callSpy.mock.calls.some(c => c.arguments[0] === 'confirm' && typeof c.arguments[1][0] === 'string' && c.arguments[1][0].includes('Choose action:') && typeof c.arguments[1][1] === 'string'))
    await session.waitFor('bufname', ['%'], new RegExp(path.basename(import.meta.filename)))
  })

  it('should select action for visual selected items', async t => {
    await manager.start(['--normal', 'location'])
    await manager.session.ui.ready
    await session.waitPrompt()
    await nvim.input('V')
    await session.waitFor('mode', [], /v/i)
    await nvim.input('2')
  ...
  it('should move cursor by <left> and <right>', async t => {
    await manager.start(['location'])
    await manager.session.ui.ready
    await session.listInput('f')
    await session.listInput('<left>')
    await session.listInput('<left>')
    await session.listInput('a')
    await session.listInput('<right>')
    await session.listInput('<right>')
    await session.listInput('c')
    let input = manager.prompt.input
    let mode = manager.prompt.mode
    manager.prompt.input = input
    manager.prompt.mode = mode
    await session.listInput('<home>')
  ...
  it('should move cursor by leftword and rightword', async t => {
    ...
    await manager.start(['location'])
    await manager.session.ui.ready
    await session.listInput('aaa bbb ccc') // -> aaa bbb ccc|
    await session.listInput('<A-b>')       // -> aaa bbb |ccc
    await session.listInput('<A-b>')       // -> aaa |bbb ccc
    await session.listInput('ddd ')        // -> aaa ddd |bbb ccc
    await session.listInput('<A-f>')       // -> aaa ddd bbb |ccc
    await session.listInput('eee ')        // -> aaa ddd bbb eee |ccc
  ...
  it('should move cursor by <end> and <home>', async t => {
    await manager.start(['location'])
    await manager.session.ui.ready
    await session.listInput('ff')
    await session.listInput('<home>')
    await session.listInput('<end>')
    await session.listInput('<end>')
    let input = manager.prompt.input
  ...
  it('should move cursor by <PageUp> <PageDown> <C-d>', async t => {
    ...
    await manager.session.ui.ready
    await session.listInput('<PageDown>')
    await session.listInput('<PageUp>')
    await session.listInput('<C-d>')
  })
  ...
  it('should change input by <Backspace>', async t => {
    ...
    await manager.session.ui.ready
    await session.listInput('f')
    await session.listInput('<backspace>')
    let input = manager.prompt.input
...
editorSuite('evalExpression', () => {
  ...
  it('should invoke normal command', async t => {
    let revert = session.updateConfiguration('list.normalMappings', {
      x: 'normal!:G'
    })
    await manager.start(['--normal', 'location'])
    await manager.session.ui.ready
    await session.listInput('x')
    revert()
