...
describe('TreeView', () => {
  ...
  describe('key-mappings', () => {
    ...
    it('should toggle selection by <space>', async t => {
      ...
      await nvim.command('exe 2')
      await nvim.input('<space>')
      await session.waitValue(() => selection?.length, 1)
      await nvim.command('exe 3')
      await nvim.input('<space>')
      await session.waitValue(async () => {
    ...
    it('should close tree view by close key', async t => {
      session.updateConfiguration('tree.key.close', 'c')
      createTreeView(defaultDef)
      await treeView.show()
      await session.wait(30)
      assert.strictEqual(treeView.visible, true)
      await nvim.input('c')
      await session.waitValue(() => treeView.visible, false)
    })
    ...
    it('should invoke command by <cr>', async t => {
      ...
      await nvim.input('<cr>')
      await session.waitValue(() => node, undefined)
      await nvim.command('exe 2')
      await nvim.input('<cr>')
      await session.waitValue(() => node && node.label, 'a')
    })
    ...
    it('should not throw when resolve command cancelled', async t => {
      ...
      await session.wait(20)
      await nvim.command('exe 1')
      await session.waitValue(() => cancelled, true)
    ...
    it('should invoke command on node click', async t => {
      ...
      await nvim.input('<LeftRelease>')
      await session.waitValue(() => node != null, true)
  ...
  describe('events', () => {
    it('should emit visibility change on buffer unload', async t => {
      ...
      await session.waitValue(() => visible, false)
    })
  ...
  describe('data change', () => {
    ...
    it('should error message on error', async t => {
      ...
      await session.waitValue(() => treeView.checkLines(), true)
      let res = await treeView.checkLines()
      assert.strictEqual(res, true)
    })
  ...
  describe('filter', () => {
    ...
    it('should start filter by input', async t => {
      ...
      await nvim.input('a')
      await session.waitFor('getline', [2], 'a ')
    })
    ...
    it('should keep state when press <cr> with empty selection', async t => {
      await createFilterTreeView()
      await nvim.input('ab')
      await session.waitValue(async () => nvim.call('getline', [1, '$']), ['test', 'ab '])
      await nvim.input('<cr>')
      await checkLines(['test', 'ab '])
    })
