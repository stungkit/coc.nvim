...
before(async () => {
  ...
  if (session.workspace.has('patch-9.0.1782')) {
...
describe('workspace', () => {
  it('should not has nvim feature', t => {
    assert.strictEqual(session.workspace.has('nvim-0.4.0'), false)
    assert.strictEqual(session.workspace.has('patch-9.0.0000'), true)
  })
  ...
  it('should evaluate dynamic insert keymaps', async t => {
    ...
    try {
      ...
      mapping.dispose()
      await session.waitValue(async () => await nvim.call('maparg', ['[', 'i']), '')
    }
  })
})
...
describe('disable and enable', () => {
  it('should keep dynamic autocmd after disable and enable', async t => {
    let times = 0
    let disposable = session.workspace.registerAutocmd({
      event: 'CursorMoved',
      request: false,
      callback: () => {
        times++
    ...
    await nvim.command('doautocmd <nomodeline> CursorMoved')
    await session.waitValue(() => times, 2)
    disposable.dispose()
  })
...
describe('vim api', () => {
  ...
  it('should show info', async t => {
    ...
    let handler = session.plugin.getHandler().workspace
  ...
  it('should navigate complete items', async t => {
    ...
    assert.strictEqual(session.completion.isActivated, true)
    await nvim.call('coc#pum#close', ['cancel'])
    await nvim.input('<esc>')
    await session.waitFor('mode', [], 'n')
    disposable.dispose()
  ...
  it('should synchronize document before completion done', async t => {
    ...
    let disposable = sources.createSource({
      ...
      onCompleteDone: (_item, opt) => {
        line = session.workspace.getDocument(opt.bufnr).getline(0)
      }
    })
    await nvim.setLine('')
    await nvim.input('i')
    ...
    try {
      ...
      await session.waitValue(() => line, 'foo')
    } finally {
      await nvim.input('<esc>')
      await session.waitFor('mode', [], 'n')
      disposable.dispose()
  ...
  it('should keep retriggered pum on noinsert navigation', async t => {
    session.updateConfiguration('suggest.noselect', true, disposables)
    ...
    try {
      ...
      feedkeys(keys)
      await session.waitValue(() => session.completion.selectedItem?.word.includes('inlayHint.position'), true)
      keys = await nvim.call('coc#pum#confirm') as string
      feedkeys(keys)
      await session.waitValue(async () => (await nvim.line).startsWith('{"inlayHint.position": '), true)
      await session.waitValue(() => session.completion.activeItems.some(item => item.word.includes('"inline"')), true)
      await session.waitFor('coc#pum#visible', [], 1)
      ...
      assert.strictEqual(session.completion.isActivated, true)
      assert.ok(session.completion.selectedItem?.word.includes('"inline"'))
    } finally {
      await nvim.input('<esc>')
      await session.waitFor('mode', [], 'n')
      disposable.dispose()
  ...
  it('should shift popup left when pumAlign is configured', async t => {
    ...
    try {
      ...
      await nvim.input('<esc>')
      await session.waitFor('mode', [], 'n')
      session.updateConfiguration('suggest.pumAlign', 'menu')
      await nvim.input('A')
      nvim.call('coc#start', { source: name }, true)
      await session.waitPopup()
      let id2 = 0
  ...
  it('should keep popup menu at word start when typed input becomes concealed', async t => {
    ...
    try {
      ...
      await nvim.input('l')
      await session.waitFor('getline', ['.'], '.'.repeat(150) + 'conceal')
  ...
  it('should keep popup menu at word start after another concealed word', async t => {
    ...
    try {
      ...
      let before = await nvim.call('popup_getpos', [id]) as { col: number }
      await nvim.input('l')
      await session.waitFor('getline', ['.'], 'conceal conceal')
      let after = await nvim.call('popup_getpos', [id]) as { col: number }
      assert.strictEqual(after.col, before.col)
      let virtcol = await nvim.call('virtcol', ['.']) as number
...
describe('client API', () => {
  it('stops and restarts a task without evaluating the Job as a number', async t => {
    ...
    await session.waitValue(async () => await nvim.call('coc#task#running', [id]), false)
  ...
  it('should input characters', async t => {
    ...
    await session.waitFor('getline', ['.'], 'abc')
    await nvim.input('<esc>')
    await session.waitFor('mode', [], 'n')
  ...
  it('keeps the centered prompt on screen with a large marginTop', async t => {
    ...
    try {
      ...
      for (let marginTop of [0, 1, 50, maxRow]) {
        let input = await session.plugin.window.createInputBox('title', '', { marginTop, position: 'center' })
  ...
  it('should out_write', async t => {
    ...
    let env = session.workspace.env
    let line = await session.getCmdline(env.lines - 1)
...
describe('Window API', () => {
  ...
  it('should add and clear matches', async t => {
    ...
    let matches = await session.getMatches('MoreMsg')
    assert.strictEqual(matches.length, 10)
    win.clearMatches(ids)
    matches = await session.getMatches('MoreMsg')
...
describe('Popup', () => {
  ...
  it('should create inputBox', async t => {
    let input = await session.plugin.window.createInputBox('title', '')
  ...
  it('updates the visible input value programmatically', async t => {
    let input = await session.plugin.window.createInputBox('title', 'old')
...
describe('document', () => {
  ...
  it('should not wait for lines when already applied', async t => {
    ...
    await assert.strictEqual(await p, undefined)
  })
  ...
  it('should synchronize changes', async t => {
    ...
    let doc = await session.createDocument(filepath)
  ...
  it('should synchronize changes after undo', async t => {
    ...
    const doc = await session.createDocument(filepath)
    nvim.pauseNotification()
  ...
  it('should synchronize changes after undo (2)', async t => {
    ...
    const doc2 = await session.createDocument(filepath2)
    nvim.pauseNotification()
    nvim.command('normal! Ofoo', true)
    nvim.command('normal! u', true)
    await nvim.resumeNotification(true)
  ...
  it('should synchronize changes after executing a command with count (2)', async t => {
    const doc2 = await session.createDocument()
    nvim.pauseNotification()
    await nvim.command('normal! 5o')
    await nvim.resumeNotification(true)
    await shouldEqual(doc2)
  ...
  it('should patch change', async t => {
    let doc = await session.workspace.document
