...
before(async () => {
  session = getSession()
  nvim = session.nvim
  handler = session.plugin.getHandler().workspace
})
afterEach(async () => {
  disposeAll(disposables)
})

editorSuite('Workspace handler', () => {
  ...
  describe('methods', () => {
    ...
    it('should open local config', async t => {
      ...
      await session.edit(path.join(os.tmpdir(), 't'))
      let root = workspace.root
      assert.strictEqual(root, os.tmpdir())
      let p = handler.openLocalConfig()
      await session.waitPromptWin()
      await nvim.input('n')
      await p
    ...
    it('should not throw when workspace folder does not exist', async t => {
      session.updateConfiguration('workspace.rootPatterns', [], disposables)
      session.updateConfiguration('workspace.ignoredFiletypes', ['vim'], disposables)
      await nvim.command('enew')
      await (window as any).openLocalConfig()
      await nvim.command(`e ${path.join(os.tmpdir(), 'a')}`)
      await session.doAction('openLocalConfig')
    ...
    it('should add workspace folder', async t => {
      ...
      await session.plugin.cocAction('addWorkspaceFolder', import.meta.dirname)
    ...
    it('should remove workspace folder', async t => {
      ...
      await session.plugin.cocAction('addWorkspaceFolder', import.meta.dirname)
      await session.plugin.cocAction('removeWorkspaceFolder', import.meta.dirname)
      let folders = workspace.workspaceFolderControl.workspaceFolders
      let uri = URI.file(import.meta.dirname).toString()
      let find = folders.find(o => o.uri === uri)
    ...
    it('should should error message for document not attached', async t => {
      ...
      await session.doAction('bufferCheck')
      await checkFloat('not attached')
      await nvim.call('coc#float#close_all', [])
      session.updateConfiguration('coc.preferences.maxFileSize', '1KB')
      await session.edit(import.meta.filename)
      await handler.bufferCheck()
      await checkFloat('not attached')
      await nvim.call('coc#float#close_all', [])
      session.updateConfiguration('coc.preferences.maxFileSize', '10MB')
    })
    ...
    it('should check json extension', async t => {
      ...
      await session.doAction('checkJsonExtension')
      await session.doAction('checkJsonExtension')
      let line = await session.getCmdline()
    ...
    it('should get rootPatterns', async t => {
      ...
      let res = await session.doAction('rootPatterns', bufnr)
      assert.notStrictEqual(res, undefined)
    })

    it('should get config by key', async t => {
      let res = await session.doAction('getConfig', ['suggest'])
      assert.notStrictEqual(res.autoTrigger, undefined)
    })

    it('should open log', async t => {
      await session.doAction('openLog')
      let bufname = await nvim.call('bufname', ['%']) as string
  ...
  describe('doKeymap()', () => {
    it('should return default value when key mapping does not exist', async t => {
      let res = await session.doAction('doKeymap', ['not_exists', ''])
