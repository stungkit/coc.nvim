...
before(async () => {
  session = getSession()
  nvim = session.nvim
  hover = session.plugin.getHandler().hover
})
...
editorSuite('Hover', () => {
  ...
  describe('onHover', () => {
    ...
    it('should show MarkupContent hover', async t => {
      session.updateConfiguration('hover.target', 'preview')
      hoverResult = { contents: { kind: 'plaintext', value: 'my hover' } }
      await session.doAction('doHover')
  ...
  describe('getHover', () => {
    it('should get hover from MarkedString array', async t => {
      ...
      let res = await session.doAction('getHover')
  ...
  describe('definitionHover', () => {
    it('should load definition from buffer', async t => {
      ...
      await session.doAction('definitionHover', 'preview')
      let res = await getDocumentText()
