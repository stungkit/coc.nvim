...
before(async () => {
  session = getSession()
  nvim = session.nvim
  callHierarchy = session.plugin.getHandler().callHierarchy
})
...
editorSuite('CallHierarchy', () => {
  ...
  it('should get incoming & outgoing callHierarchy items', async t => {
    ...
    let res = await session.doAction('incomingCalls')
    assert.strictEqual(res.length, 1)
    assert.strictEqual(res[0].from.name, 'bar')
    let outgoing = await session.doAction('outgoingCalls')
  ...
  it('should show warning when provider does not exist', async t => {
    await session.doAction('showIncomingCalls')
  ...
  it('should render description and support default action', async t => {
    session.updateConfiguration('callHierarchy.enableTooltip', false)
    let doc = await workspace.document
    let bufnr = doc.bufnr
    ...
    await nvim.input('t')
    await session.waitFor('getline', ['.'], '  - c bar Detail')
    await nvim.input('<cr>')
    await session.waitFor('expand', ['%:p'], fsPath)
    ...
    await session.waitValue(async () => (await nvim.call('getmatches') as any[]).length, 0)
  ...
  it('should invoke reveal command', async t => {
    ...
    await commands.executeCommand(commandId, winid, item)
    let newDoc = await session.createDocument('bar')
  ...
  it('should invoke show incoming calls action', async t => {
    ...
    await session.waitPrompt()
    await nvim.input('3')
    await session.waitValue(async () => buf.lines, [
      'INCOMING CALLS',
      '- c bar Detail',
      '  + c test'
    ])
  ...
  it('should invoke dismiss action #2', async t => {
    ...
    await session.doAction('showOutgoingCalls')
    let buf = await nvim.buffer
    let lines = await buf.lines
    assert.deepStrictEqual(lines, [
      'OUTGOING CALLS',
    ...
    await nvim.command('exe 3')
    await nvim.input('t')
    await session.waitFor('line', ['$'], 4)
    await nvim.command('exe 4')
    await nvim.input('<tab>')
    await session.waitPrompt()
    await nvim.input('2')
    await session.waitFor('line', ['$'], 3)
    lines = await buf.lines
    assert.deepStrictEqual(lines, [
      'OUTGOING CALLS',
      '- c foo',
