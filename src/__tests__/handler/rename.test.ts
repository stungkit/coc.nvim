...
before(async () => {
  session = getSession()
  nvim = session.nvim
  rename = session.plugin.getHandler().rename
})
...
editorSuite('rename handler', () => {
  describe('getWordEdit', () => {
    ...
    it('should extract words from buffer', async t => {
      let doc = await session.createDocument('t')
  ...
  describe('rename', () => {
    it('should throw when provider not found', async t => {
      await session.edit()
      await assert.rejects(session.doAction('rename', 'foo'), Error)
    })
    ...
    it('should return false for empty name', async t => {
      session.updateConfiguration('coc.preferences.renameFillCurrent', false)
      await session.createDocument('t.js')
      await nvim.setLine('foo foo foo')
      let p = rename.rename()
      await session.waitFloat()
      await nvim.input('<C-u>')
    ...
    it('should use newName from range', async t => {
      ...
      await session.waitFor('getline', ['.'], 'bar bar bar')
    })
