...
before(async () => {
  ...
  locations = session.plugin.getHandler().locations
})
...
editorSuite('locations', () => {
  ...
  describe('reference', () => {
    ...
    it('should get references', async t => {
      ...
      let res = await session.doAction('references')
      assert.strictEqual(res.length, 2)
    })

    it('should jump to references', async t => {
      currLocations = [createLocation('foo', 0, 0, 0, 0)]
      let res = await session.doAction('jumpReferences', 'edit')
    ...
    it('should return false when references not found', async t => {
      ...
      res = await session.doAction('jumpUsed', 'edit')
      assert.strictEqual(res, false)
    })
  })
  ...
  describe('definition', () => {
    ...
    it('should get definitions', async t => {
      ...
      let res = await session.doAction('definitions')
      assert.strictEqual(res.length, 2)
    })
    ...
    it('should jump to definitions', async t => {
      currLocations = [createLocation('foo', 0, 0, 0, 0)]
      let res = await session.doAction('jumpDefinition', 'edit')
      assert.strictEqual(res, true)
      let name = await nvim.call('bufname', ['%'])
      assert.strictEqual(name, 'test://foo')
    })
  ...
  describe('declaration', () => {
    ...
    it('should return false when declaration not found', async t => {
      currLocations = []
      let res = await session.doAction('jumpDeclaration', 'edit')
      assert.strictEqual(res, false)
    })
  })
  ...
  describe('typeDefinition', () => {
    ...
    it('should get type definition', async t => {
      currLocations = [createLocation('foo', 0, 0, 0, 0), createLocation('bar', 0, 0, 0, 0)]
      let res = await session.doAction('typeDefinitions')
      assert.strictEqual(res.length, 2)
    })
    ...
    it('should return false when type definition not found', async t => {
      currLocations = []
      let res = await session.doAction('jumpTypeDefinition', 'edit')
      assert.strictEqual(res, false)
    })
  })
  ...
  describe('implementation', () => {
    ...
    it('should get implementations', async t => {
      currLocations = [createLocation('foo', 0, 0, 0, 0), createLocation('bar', 0, 0, 0, 0)]
      let res = await session.doAction('implementations')
      assert.strictEqual(res.length, 2)
    })

    it('should jump to implementation', async t => {
      currLocations = [createLocation('foo', 0, 0, 0, 0)]
      let res = await session.doAction('jumpImplementation', 'edit')
      assert.strictEqual(res, true)
      let name = await nvim.call('bufname', ['%'])
      assert.strictEqual(name, 'test://foo')
    })
  ...
  describe('getTagList', () => {
    it('should return null when cword does not exist', async t => {
      let res = await session.doAction('getTagList')
  ...
  describe('findLocations', () => {
    ...
    it('should handle locations from language client', async t => {
      ...
      await session.doAction('findLocations', 'foo', 'mylocation', {}, false)
