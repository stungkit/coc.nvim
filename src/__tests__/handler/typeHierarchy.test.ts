...
before(async () => {
  session = getSession()
  nvim = session.nvim
  handler = session.plugin.getHandler().typeHierarchy
})
...
editorSuite('TypeHierarchy', () => {
  ...
  describe('TypeHierarchyHandler', () => {
    ...
    it('should invoke super types and sub types action', async t => {
      ...
      await handler.showTypeHierarchyTree('supertypes')
      await session.waitFor('getline', [2], '- c foo')
      await nvim.command('exe 2')
      await nvim.input('<tab>')
      await session.waitPrompt()
      await nvim.input('4')
      await session.waitFor('getline', [1], 'Sub types')
      await nvim.input('<tab>')
      await session.waitPrompt()
      await nvim.input('3')
      await session.waitFor('getline', [1], 'Super types')
    })
