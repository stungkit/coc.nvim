...
before(async () => {
  session = getSession()
  nvim = session.nvim
  inlineCompletion = session.plugin.handler.inlineCompletion
})
...
editorSuite('InlineCompletion', () => {
  ...
  describe('events', () => {
    it('should trigger on document change', async t => {
      session.updateConfiguration('inline.autoTrigger', true, disposables)
    ...
    it('should trigger on pum navigate', async t => {
      ...
      await session.waitFor('coc#inline#visible', [], 1)
  ...
  describe('accept()', () => {
    it('should not accept when no selected item', async t => {
      ...
      let res = await session.doAction('inlineAccept', doc.bufnr, 'all')
  ...
  describe('trigger()', () => {
    ...
    beforeEach(async (t: any) => {
      ...
      t.mock.method(session.plugin.handler, 'getCurrentState', async () => ({
    ...
    it('should not trigger if no provider is registered for the document', async t => {
      ...
      let doc = await workspace.document
      await session.doAction('inlineTrigger', doc.bufnr)
    ...
    it('should not trigger if token is cancelled before provider call', async t => {
      ...
      await session.doAction('inlineCancel')
  ...
  describe('next and prev', () => {
    ...
    describe('next()', () => {
      ...
      it('should loop to the first item when at the last item', async t => {
        ...
        await getSession().doAction('inlineNext', bufnr)
        assert.strictEqual(session.index, 0)
    ...
    describe('prev()', () => {
      ...
      it('should move to the previous item and call insertVtext', async t => {
        ...
        await getSession().doAction('inlinePrev', bufnr)
        assert.strictEqual(session.index, 0)
        assert.deepStrictEqual(mockInsertVtext.mock.calls[0].arguments, [item1])
      })
