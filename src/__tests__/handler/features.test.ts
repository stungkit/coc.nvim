...
before(async () => {
  session = getSession()
  nvim = session.nvim
  handler = (session.plugin as any).handler
  links = session.plugin.getHandler().links
  highlights = session.plugin.handler.documentHighlighter
  folds = session.plugin.getHandler().fold
  linkedEditingHandler = session.plugin.getHandler().linkedEditingHandler
  refactor = session.plugin.getHandler().refactor
})
...
async function matches(): Promise<number> {
  let list = await session.getMatches('CocLinkedEditing')
  return list.length
}
...
editorSuite('Handler', () => {
  ...
  describe('hasProvider', () => {
    it('should check provider for document', async t => {
      let res = await session.doAction('hasProvider', 'definition')
  ...
  describe('getIcon', () => {
    it('should get icon', t => {
      session.updateConfiguration('suggest.completionItemKindLabels', {
        default: 'd'
      })
...
editorSuite('Links', () => {
  ...
  it('should get document links', async t => {
    ...
    let res = await session.doAction('links')
  ...
  it('should cancel resolve on InsertEnter', async t => {
    session.updateConfiguration('links.tooltip', true)
    let doc = await workspace.document
    let called = false
    let cancelled = false
    disposables.push(languages.registerDocumentLinkProvider([{ language: '*' }], {
  ...
  it('should open link at current position', async t => {
    ...
    await session.doAction('openLink')
...
editorSuite('LinkBuffer', () => {
  ...
  it('should do highlight', async t => {
    ...
    session.updateConfiguration('links.highlight', true)
    let doc = await session.createDocument()
    await nvim.setLine('foo')
    await doc.synchronize()
    let buf = links.getBuffer(doc.bufnr)
...
editorSuite('document highlights', () => {
  ...
  it('should cancel on timeout', async t => {
    session.updateConfiguration('documentHighlight.timeout', 10)
    let fn = t.mock.fn()
    registerTimerProvider(fn, 3000)
    await session.edit()
    await nvim.setLine('foo')
  ...
  it('should add highlights to symbols', async t => {
    ...
    await session.doAction('highlight')
  ...
  it('should return highlight ranges', async t => {
    ...
    let res = await session.doAction('symbolRanges')
    assert.strictEqual(res.length, 2)
  })
...
editorSuite('Folds', () => {
  ...
  it('should return false when no fold ranges found', async t => {
    ...
    await session.wait(20)
    let res = await session.doAction('fold')
    assert.strictEqual(res, false)
  })
...
editorSuite('LinkedEditing', () => {
  beforeEach(async () => {
    session.updateConfiguration('coc.preferences.enableLinkedEditing', true)
  })
  ...
  it('should not cancel when insert line break before range', async t => {
    ...
    await session.waitValue(() => matches(), 2)
  })
...
editorSuite('search', () => {
  ...
  it('should work with CocAction search', async t => {
    await session.doAction('search', ['CocAction'])
  ...
  it('should show empty result when no result found', async t => {
    ...
    await session.doAction('search', ['should found ' + keyword])
    let bufnr = await nvim.call('bufnr', ['%']) as number
    let buf = refactor.getBuffer(bufnr)
    assert.notStrictEqual(buf, undefined)
