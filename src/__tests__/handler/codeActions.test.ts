...
before(async () => {
  session = getSession()
  nvim = session.nvim
  codeActions = session.plugin.getHandler().codeActions
})
...
editorSuite('handler codeActions', () => {
  describe('autoApply', () => {
  ...
  describe('organizeImport', () => {
    ...
    it('should return false when organize import action not found', async t => {
      ...
      let res = await session.doAction('organizeImport')
      assert.strictEqual(res, false)
  ...
  describe('codeActionRange', () => {
    it('should show warning when no action available', async t => {
      ...
      currActions = []
      await session.doAction('codeActionRange', 1, 2, CodeActionKind.QuickFix)
      let line = await session.getCmdline()
      assert.match(line, /No quickfix code action/)
      await session.doAction('codeActionRange', 1, 2)
    ...
    it('should show command tooltip in code action menu', async t => {
      ...
      let p = session.doAction('codeAction', undefined)
      await session.waitPrompt()
      let win = await session.getFloat()
      assert.notStrictEqual(win, undefined)
      let lines = await session.getWinLines(win.id)
  ...
  describe('getCurrentCodeActions', () => {
    ...
    it('should get codeActions by line', async t => {
      currActions = []
      await session.createDocument()
      let res = await session.doAction('codeActions', 'line')
  ...
  describe('doCodeAction', () => {
    it('should not throw when no action exists', async t => {
      currActions = []
      await session.createDocument()
      await session.doAction('codeAction', undefined)
    })
    ...
    it('should use quickpick', async t => {
      session.updateConfiguration('coc.preferences.floatActions', false)
      ...
      session.updateConfiguration('coc.preferences.floatActions', true)
    })
  ...
  describe('doQuickfix', () => {
    it('should show message when quickfix action does not exist', async t => {
      currActions = []
      await session.createDocument()
      await session.doAction('doQuickfix')
  ...
  describe('applyCodeAction', () => {
    it('should resolve codeAction', async t => {
      ...
      let arr = await session.doAction('quickfixes', 'line')
    ...
    it('should throw for disabled action', async t => {
      ...
      await assert.rejects(session.doAction('doCodeAction', action), Error)
    })
  ...
  it('should execute code action with timeout', async t => {
    disposeAll(disposables)
    let doc = await session.createDocument('t.js')
    let called = false
    disposables.push(languages.registerCodeActionProvider([{ language: '*' }], {
      provideCodeActions: (
        _document: TextDocument,
