...
before(async () => {
  ...
  codeLens = session.plugin.getHandler().codeLens
})

beforeEach(() => {
  session.updateConfiguration('codeLens.enable', true)
})
...
async function createBufferWithCodeLens(): Promise<CodeLensBuffer> {
  ...
  let doc = await session.createDocument('e.js')
...
editorSuite('codeLenes feature', () => {
  ...
  it('should invoke codeLenes action', async t => {
    ...
    await session.doAction('codeLensAction')
  ...
  it('should change codeLenes position', async t => {
    session.updateConfiguration('codeLens.position', 'eol')
  ...
  it('should refresh codeLens on CursorHold', async t => {
    ...
    let doc = await session.createDocument('example.js')
    await nvim.call('setline', [1, ['a', 'b', 'c']])
    await doc.synchronize()
    ...
    session.updateConfiguration('codeLens.enable', false)
    await events.fire('CursorHold', [doc.bufnr])
  })
  ...
  it('should cancel codeLenes request on document change', async t => {
    ...
    let doc = await session.createDocument('codelens.js')
    await session.waitValue(() => started, true)
  ...
  it('should use picker for multiple codeLenses', async t => {
    ...
    let p = session.doAction('codeLensAction')
  ...
  it('should refresh for failed codeLens request', async t => {
    ...
    await session.waitValue(() => codeLensBuffer.currentCodeLens.length > 1, true)
    let curr = codeLensBuffer.currentCodeLens
    assert.ok(curr.length > 1)
  })

  it('should use custom separator & position', async t => {
    session.updateConfiguration('codeLens.separator', '|')
    session.updateConfiguration('codeLens.position', 'eol')
    let doc = await session.createDocument('example.js')
    await nvim.call('setline', [1, ['a', 'b', 'c']])
    await doc.synchronize()
