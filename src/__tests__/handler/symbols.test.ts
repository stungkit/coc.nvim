...
editorSuite('symbols handler', () => {
  ...
  describe('configuration', () => {
    it('should get configuration', async t => {
      ...
      session.updateConfiguration('coc.preferences.currentFunctionSymbolAutoUpdate', true)
    ...
    it('should update symbols automatically', async t => {
      ...
      await session.waitFor('eval', ['get(b:,"coc_current_function","")'], 'fun1')
      await events.fire('CursorMoved', [buf.id, [1, 8]])
      await session.waitFor('eval', ['get(b:,"coc_current_function","")'], 'myClass')
    })
  })
  ...
  describe('documentSymbols', () => {
    ...
    it('should get symbols of current buffer', async t => {
      ...
      let res = await session.plugin.cocAction('documentSymbols')
      assert.strictEqual(res.length, 2)
    ...
    `
      ...
      let res = await session.doAction('getCurrentFunctionSymbol')
      assert.strictEqual(res, 'fun1')
      await nvim.command('normal! G')
      res = await session.doAction('getCurrentFunctionSymbol')
  ...
  describe('selectSymbolRange', () => {
    ...
    it('should select symbol range at cursor position', async t => {
      ...
      await createBuffer(code)
      await nvim.call('cursor', [3, 0])
      await session.doAction('selectSymbolRange', false, '', ['Function', 'Method'])
    ...
    it('should select symbol range from select range', async t => {
      ...
      await nvim.input('<esc>')
      await session.doAction('selectSymbolRange', false, 'v', ['Class'])
      let mode = await nvim.mode
      assert.strictEqual(mode.mode, 'v')
  ...
  describe('cancel', () => {
    it('should cancel symbols request on insert', async t => {
      ...
      let doc = await session.createDocument('t.txt')
  ...
  describe('workspaceSymbols', () => {
    it('should get workspace symbols', async t => {
      ...
      let resolved = await session.doAction('resolveWorkspaceSymbol', res[0])
    ...
    it('should return symbol when resolve failed', async t => {
      ...
      let res = await session.doAction('getWorkspaceSymbols')
