...
before(async () => {
  session = getSession()
  nvim = session.nvim
  handler = session.plugin.getHandler().inlayHintHandler
...
editorSuite('InlayHint', () => {
  ...
  describe('env & options', () => {
    it('should not enabled when disabled by configuration', async t => {
      session.updateConfiguration('inlayHint.filetypes', [], disposables)
      let doc = await workspace.document
      let item = handler.getItem(doc.bufnr)
      item.clearVirtualText()
      assert.strictEqual(item.enabled, false)
      session.updateConfiguration('inlayHint.filetypes', ['dos'], disposables)
  ...
  describe('configuration', () => {
    it('should refresh on insert mode', async t => {
      session.updateConfiguration('inlayHint.refreshOnInsertMode', true, disposables)
    ...
    it('should disable parameter inlayHint', async t => {
      session.updateConfiguration('inlayHint.enableParameter', false, disposables)
      let doc = await session.createDocument()
      let disposable = await registerProvider('foo\nbar')
      disposables.push(disposable)
      await waitRefresh(doc.bufnr)
    ...
    it('should enable & disable inlayHint', async t => {
      ...
      disposables.push(disposable)
      await waitRefresh(doc.bufnr)
      session.updateConfiguration('inlayHint.enable', false)
      let markers = await doc.buffer.getExtMarks(ns, 0, -1, { details: true })
      assert.strictEqual(markers.length, 0)
      session.updateConfiguration('inlayHint.enable', true)
    })

    it('should change position to eol', async t => {
      session.updateConfiguration('inlayHint.position', 'eol', disposables)
      let doc = await session.createDocument()
      let disposable = await registerProvider('foo\nbar')
      disposables.push(disposable)
      await waitRefresh(doc.bufnr)
    ...
    it('should truncate hint label when exceeding maximumLength', async t => {
      session.updateConfiguration('inlayHint.maximumLength', 13, disposables)
      let doc = await session.createDocument()
      let disposable = languages.registerInlayHintsProvider([{ language: '*' }], {
        provideInlayHints: () => {
          return [
    ...
    it('should not truncate hint label when maximumLength is 0', async t => {
      session.updateConfiguration('inlayHint.maximumLength', 0, disposables)
      let doc = await session.createDocument()
      let disposable = languages.registerInlayHintsProvider([{ language: '*' }], {
        provideInlayHints: () => {
          return [
  ...
  describe('render()', () => {
    ...
    it('should resend request on CancellationError', async t => {
      ...
      await session.createDocument(filepath)
      await nvim.command('setfiletype vim')
      await session.waitValue(() => called, 2)
    })
  })
})
