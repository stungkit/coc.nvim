...
before(async () => {
  session = getSession()
  nvim = session.nvim
  signature = session.plugin.getHandler().signature
})
...
editorSuite('signatureHelp', () => {
  ...
  describe('triggerSignatureHelp', () => {
    ...
    it('should use 0 when activeParameter is undefined', async t => {
      ...
      await session.createDocument()
      await nvim.input('foo')
      await session.doAction('showSignatureHelp')
  ...
  describe('events', () => {
    ...
    it('should close signature float when empty signatures returned', async t => {
      ...
      empty = true
      await signature.triggerSignatureHelp()
      await session.waitValue(() => nvim.call('coc#float#valid', [win.id]), 0)
  ...
  describe('configurations', () => {
    ...
    it('should hide signature window on text change', async t => {
      ...
      let winid = await session.waitFloat()
      await nvim.input('x')
      await session.waitValue(() => nvim.call('coc#float#valid', [winid]), 0)
