...
before(async () => {
  session = getSession()
  nvim = session.nvim
  selection = session.plugin.getHandler().selectionRange
  commands = session.plugin.handler.commands
})
...
editorSuite('selectionRange', () => {
  describe('getSelectionRanges()', () => {
    it('should throw error when selectionRange provider does not exist', async t => {
      ...
      await assert.rejects(session.doAction('selectionRanges'), Error)
    })
  ...
  describe('selectRange()', () => {
    ...
    it('should select ranges forward', async t => {
      ...
      await doc.synchronize()
      await session.doAction('rangeSelect', '', false)
...
editorSuite('Commands', () => {
  ...
  describe('addVimCommand', () => {
    it('should register global vim commands', async t => {
      ...
      let list = await session.doAction('commandList')
      assert.strictEqual(list.includes('vim.config'), true)
    })

    it('should add vim command with title', async t => {
      await session.plugin.cocAction('addCommand', { id: 'bad', cmd: '', title: '' })
  ...
  describe('getCommands', () => {
    it('should get command items', async t => {
      let res = await session.doAction('commands')
  ...
  describe('repeat', () => {
    it('should repeat command', async t => {
      ...
      await session.doAction('runCommand', 'vim.remove')
      await session.waitFor('getline', ['.'], 'b')
      await session.doAction('repeatCommand')
      await session.waitFor('getline', ['.'], 'c')
    })
  })
