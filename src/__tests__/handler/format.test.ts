...
before(async () => {
  session = getSession()
  nvim = session.nvim
  format = session.plugin.getHandler().format
})

beforeEach(() => {
  session.updateConfiguration('coc.preferences.formatOnType', true)
})
...
editorSuite('format handler', () => {
  describe('documentFormat', () => {
    ...
    it('should use specified format provider', async t => {
      session.updateConfiguration('coc.preferences.formatterExtension', 'foo', disposables)
  ...
  describe('rangeFormat', () => {
    ...
    it('should invoke range format', async t => {
      ...
      await session.doAction('formatSelected', 'v')
  ...
  describe('formatOnType', () => {
    it('should invoke format', async t => {
      ...
      await session.doAction('format')
      let line = await nvim.line
      assert.deepStrictEqual(line, '  foo')
    })

    it('should respect formatOnTypeFiletypes', async t => {
      session.updateConfiguration('coc.preferences.formatOnTypeFiletypes', ['*'])
      assert.strictEqual(format.shouldFormatOnType('vim'), true)
      session.updateConfiguration('coc.preferences.formatOnTypeFiletypes', ['txt'])
      let doc = await session.createDocument('t.vim')
    ...
    it('should adjust cursor after format on type', async t => {
      ...
      await nvim.setLine('"')
      await nvim.input('i|')
      await session.waitFor('getline', ['.'], '  |"end')
      let cursor = await window.getCursorPosition()
      assert.deepStrictEqual(cursor, { line: 0, character: 3 })
    })
  })
  ...
  describe('bracketEnterImprove', () => {
    ...
    it('should format vim file on enter', async t => {
      let buf = await session.edit('foo.vim')
      ...
      await session.waitFor('getline', [2], '  \\ ')
      let lines = await buf.lines
